...
export default { getEmailById, getEmails, createEmails, addEmail, deleteEmail, markAsRead }
...
function getEmails(filterBy) {
    if (filterBy) return Promise.resolve(gEmails.filter(email => {
        return email.subject.includes(filterBy) || email.body.includes(filterBy.body)
    }))
